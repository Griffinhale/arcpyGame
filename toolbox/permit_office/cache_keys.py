...
def _state_rules_row(state):
    return {
        ...
        "stakeholder_heat": _string_keyed_mapping(getattr(state, "stakeholder_heat", {}) or {}),
        "stakeholder_memory": _string_keyed_mapping(getattr(state, "stakeholder_memory", {}) or {}),
        "maintenance_backlog": int(getattr(state, "maintenance_backlog", 0) or 0),
        "pending_followups": _string_keyed_mapping(getattr(state, "pending_followups", {}) or {}),
    }
...
def _district_rules_row(profile):
    return {
        ...
        "dissatisfaction": _string_keyed_mapping(getattr(profile, "dissatisfaction", {}) or {}),
        "hazards": _string_keyed_mapping(getattr(profile, "hazards", {}) or {}),
        "network_access": _string_keyed_mapping(getattr(profile, "network_access", {}) or {}),
        "service_gap": _string_keyed_mapping(getattr(profile, "service_gap", {}) or {}),
...
def _docket_row(item):
    return {
        ...
        "target_cell_ids": _string_list(getattr(item, "target_cell_ids", []) or []),
...
def _feature_rules_row(feature):
    return {
        ...
        "target_cell_ids": _string_list(getattr(feature, "target_cell_ids", []) or []),
...
def _project_row(project):
    return {
        ...
        "target_cell_ids": _string_list(getattr(project, "target_cell_ids", []) or []),
    }


def _string_keyed_mapping(value) -> dict[str, object]:
    if isinstance(value, Mapping):
        items = value.items()
    else:
        try:
    ...
    for entry in items:
        try:
            ...
            key, item = repr(entry), True
        out[str(key)] = _canonical(item)
    return out
